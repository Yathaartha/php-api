<?php
  class UserController extends BaseController{
    ...
    public function listAction(){
      ...
      /**
        * "/user/edit" Endpoint - Create new user
        */
        ...
        public function editAction(){
          ...
          // if(strtoupper($requestMethod) == 'POST'){
            try{
              $userModel = new UserModel();
              $arrUser = $userModel->editUser($arrFormParams['customerid'], $arrFormParams['firstname'], $arrFormParams['lastname'], $arrFormParams['username'], $arrFormParams['address'], $arrFormParams['phone'], $arrFormParams['email'], $arrFormParams['password'], $arrFormParams['image']);
              $responseData = json_encode($arrUser);
            } catch(Error $e){
              $strErrorDesc = $e->getMessage().'Something went wrong! Please contact supper.';
              $strErrorHeader = 'HTTP/1.1 500 Internal Server Error';
